...
object AdiDewasaHelper {
    ...
    val headers = mapOf(
        ...
        "Origin" to "https://dramafull.cc"
    )

    // Fungsi normalisasi judul (Menghapus tahun, simbol, dll)
    fun normalizeQuery(title: String): String {
        return title
            .replace(Regex("\\(\\d{4}\\)"), "") // Hapus tahun dalam kurung
            .replace(Regex("[^a-zA-Z0-9\\s]"), " ") // Hapus simbol aneh
            .trim()
    ...
    fun isFuzzyMatch(original: String, result: String): Boolean {
        ...
        // Jika salah satu judul sangat pendek, harus match persis
        if (cleanOrg.length < 4 || cleanRes.length < 4) {
            return cleanOrg == cleanRes
        }

        // Cek apakah mengandung kata yang sama
        return cleanRes.contains(cleanOrg) || cleanOrg.contains(cleanRes)
    }
}
// ============================================================
