...
object AboruFilmExtractor : AboruFilm() {
    ...
    suspend fun invokeInternalSource(
        ...
        // Menggunakan Token Hardcoded di payload JSON
        val query = if (type == ResponseTypes.Movies.value) {
            """{"childmode":"0","uid":"","app_version":"11.5","appid":"$appId","module":"Movie_downloadurl_v3","channel":"Website","mid":"$id","lang":"","expired_date":"${getExpiryDate()}","platform":"android","uid":"$HARDCODED_TOKEN","open_udid":"$HARDCODED_TOKEN","group":""}"""
        } else {
            """{"childmode":"0","app_version":"11.5","module":"TV_downloadurl_v3","channel":"Website","episode":"$episode","expired_date":"${getExpiryDate()}","platform":"android","tid":"$id","oss":"1","uid":"$HARDCODED_TOKEN","open_udid":"$HARDCODED_TOKEN","appid":"$appId","season":"$season","lang":"en","group":""}"""
        }
        ...
        val subtitleQuery = if (type == ResponseTypes.Movies.value) {
            """{"childmode":"0","fid":"$fid","uid":"","app_version":"11.5","appid":"$appId","module":"Movie_srt_list_v2","channel":"Website","mid":"$id","lang":"en","uid":"$HARDCODED_TOKEN","open_udid":"$HARDCODED_TOKEN","expired_date":"${getExpiryDate()}","platform":"android"}"""
        } else {
            """{"childmode":"0","fid":"$fid","app_version":"11.5","module":"TV_srt_list_v2","channel":"Website","episode":"$episode","expired_date":"${getExpiryDate()}","platform":"android","tid":"$id","uid":"$HARDCODED_TOKEN","open_udid":"$HARDCODED_TOKEN","appid":"$appId","season":"$season","lang":"en"}"""
        }
    ...
    suspend fun invokeExternalSource(
        ...
        fids.amapIndexed { index, fileList ->
            // Menggunakan Cookie Token Hardcoded
            val superToken = HARDCODED_COOKIE_TOKEN
            ...
            listOf(jsonObject.toString()).forEach {
                ...
                parsedSources.forEach org@{ source ->
                    ...
                    callback.invoke(
                        newExtractorLink(
                            "⌜ AboruFilm ⌟ External",
                            "⌜ AboruFilm ⌟ External [Server ${index + 1}] ${source.size}",
    ...
    suspend fun invokeWatchsomuch(
        ...
        val epsId = app.post(
            ...
            data = mapOf(
                "index" to "0",
                "mid" to "$id",
                "wsk" to "30fb68aa-1c71-4b8c-b5d4-4ca9222cfb45", // Key ini sudah hardcoded dari awal
                "lid" to "",
                "liu" to ""
