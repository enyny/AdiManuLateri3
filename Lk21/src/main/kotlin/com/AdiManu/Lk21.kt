...
class Lk21 : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        ...
        "$mainUrl/genre/action/page/" to "Film Aksi",
        "$mainUrl/genre/horror/page/" to "Film Horor",
        "$mainUrl/genre/drama/page/" to "Film Drama"
    )
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val year = Regex("\\d, (\\d+)").find(document.select("div.movie-info h1").text().trim())?.groupValues?.get(1)?.toIntOrNull()
